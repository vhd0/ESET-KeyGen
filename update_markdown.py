...
def parse_output_content(content): # Hàm này nhận nội dung trực tiếp
    """
    Parses the output content string to extract key information.
    """
    # Regex để bắt các thông tin cần thiết, đã điều chỉnh cho định dạng log
    # Nó khớp với thứ tự quan sát và bỏ qua các số dòng một cách rõ ràng
    pattern = re.compile(
...
if __name__ == "__main__":
    ...
    if parsed_data:
        ...
        print(f"Đã cập nhật generated_results.md thành công với dữ liệu từ nội dung được cung cấp.")
    else:
        print(f"Không tìm thấy thông tin khóa license mới trong nội dung được cung cấp.")
